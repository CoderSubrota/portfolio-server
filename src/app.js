...
// const uri = `mongodb+srv://${username}:${password}@cluster0.dfdww.mongodb.net/`;

const uri = `mongodb+srv://${username}:${password}@cluster0.dfdww.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;

const client = new MongoClient(uri, { 
    useNewUrlParser: true, 
    useUnifiedTopology: true, 
    serverApi: { version: ServerApiVersion.v1 } // Correct usage of ServerApiVersion
});
...
} else if(userData?.email === "itsectorcommunication@gmail.com") {
    const result = await users.insertOne({
        ...userData , 
         role : "admin" , 
    }) ;
    console.info("admin =>",result) ;
    return  res.status(201).send(result) ;
} else{
